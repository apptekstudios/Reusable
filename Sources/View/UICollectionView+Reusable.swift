...
public extension UICollectionView {
  ...
  final func dequeueReusableCell<T: UICollectionViewCell>(for indexPath: IndexPath, cellType: T.Type = T.self) -> T
    where T: Reusable & AutoRegistering {
      return dequeueReusableCell(for: indexPath, registerClosure: {
  ...
  final func dequeueReusableCell<T: UICollectionViewCell>(for indexPath: IndexPath, cellType: T.Type = T.self) -> T
    where T: Reusable {
      return dequeueReusableCell(for: indexPath, registerClosure: nil)
  }

  final private func dequeueReusableCell<T: UICollectionViewCell>(for indexPath: IndexPath,
                                                                          cellType: T.Type = T.self,
                                                                          registerClosure: (() -> Void)?) -> T
    where T: Reusable {
      if let registerClosure = registerClosure {
        if let bareCell = try? ObjCException.catchExceptions({
          return self.dequeueReusableCell(withReuseIdentifier: cellType.reuseIdentifier, for: indexPath)
        }), let cell = bareCell as? T {
          return cell
        } else {
          registerClosure()
        }
  ...
  final func dequeueReusableSupplementaryView<T: UICollectionReusableView>
    ...
    where T: Reusable & AutoRegistering {
      return dequeueReusableSupplementaryView(ofKind: elementKind, for: indexPath, registerClosure: {
        self.register(supplementaryViewType: viewType, ofKind: elementKind)
      })
  }
  ...
  final func dequeueReusableSupplementaryView<T: UICollectionReusableView>
    (ofKind elementKind: String, for indexPath: IndexPath, viewType: T.Type = T.self) -> T
    where T: Reusable {
      return dequeueReusableSupplementaryView(ofKind: elementKind, for: indexPath, registerClosure: nil)
  }

  final private func dequeueReusableSupplementaryView<T: UICollectionReusableView>
    ...
    where T: Reusable {
      if let registerClosure = registerClosure {
        if let bareView = try? ObjCException.catchExceptions({
          return self.dequeueReusableSupplementaryView(ofKind: elementKind, withReuseIdentifier: viewType.reuseIdentifier, for: indexPath
          )
        }), let view = bareView as? T {
          return view
        } else {
          registerClosure()
        }
